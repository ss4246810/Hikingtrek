...
function travel_agency_pro_fonts_url(){
    $fonts_url = '';
    
    $primary_font       = get_theme_mod( 'primary_font', 'Poppins' );
    $ig_primary_font    = travel_agency_pro_is_google_font( $primary_font );    
    $secondary_font     = get_theme_mod( 'secondary_font', 'Montserrat' );
    $ig_secondary_font  = travel_agency_pro_is_google_font( $secondary_font );    
    
    $h1_font    = get_theme_mod( 'h1_font', array( 'font-family'=>'Montserrat', 'variant'=>'700' ) );
    $ig_h1_font = travel_agency_pro_is_google_font( $h1_font['font-family'] );
    $h2_font    = get_theme_mod( 'h2_font', array( 'font-family'=>'Montserrat', 'variant'=>'700' ) );
    $ig_h2_font = travel_agency_pro_is_google_font( $h2_font['font-family'] ); 
    $h3_font    = get_theme_mod( 'h3_font', array( 'font-family'=>'Montserrat', 'variant'=>'700' ) );
    $ig_h3_font = travel_agency_pro_is_google_font( $h3_font['font-family'] );
    $h4_font    = get_theme_mod( 'h4_font', array( 'font-family'=>'Montserrat', 'variant'=>'700' ) );
    $ig_h4_font = travel_agency_pro_is_google_font( $h4_font['font-family'] );
    $h5_font    = get_theme_mod( 'h5_font', array( 'font-family'=>'Montserrat', 'variant'=>'700' ) );
    $ig_h5_font = travel_agency_pro_is_google_font( $h5_font['font-family'] );
    $h6_font    = get_theme_mod( 'h6_font', array( 'font-family'=>'Montserrat', 'variant'=>'700' ) );
    $ig_h6_font = travel_agency_pro_is_google_font( $h6_font['font-family'] );    
    
    /* Translators: If there are characters in your language that are not
    * supported by respective fonts, translate this to 'off'. Do not translate
    * into your own language.
    */
    $primary    = _x( 'on', 'Primary Font: on or off', 'travel-agency-pro' );
    $secondary  = _x( 'on', 'Secondary Font: on or off', 'travel-agency-pro' );
    $h1         = _x( 'on', 'H1 Content Font: on or off', 'travel-agency-pro' );
    $h2         = _x( 'on', 'H2 Content Font: on or off', 'travel-agency-pro' );
    $h3         = _x( 'on', 'H3 Content Font: on or off', 'travel-agency-pro' );
    $h4         = _x( 'on', 'H4 Content Font: on or off', 'travel-agency-pro' );
    $h5         = _x( 'on', 'H5 Content Font: on or off', 'travel-agency-pro' );
    $h6         = _x( 'on', 'H6 Content Font: on or off', 'travel-agency-pro' );
    
    if ( 'off' !== $primary || 'off' !== $secondary || 'off' !== $h1 || 'off' !== $h2 || 'off' !== $h3 || 'off' !== $h4 || 'off' !== $h5 || 'off' !== $h6 ) {
        
        $font_families = array();
     
        if ( 'off' !== $primary && $ig_primary_font ) {
            $primary_variant = travel_agency_pro_check_varient( $primary_font, 'regular', true );
            if( $primary_variant ){
        ...
        if ( 'off' !== $secondary && $ig_secondary_font ) {
            $secondary_variant = travel_agency_pro_check_varient( $secondary_font, 'regular', true );
        ...
        if ( 'off' !== $h1 && $ig_h1_font ) {
            if( ! empty( $h1_font['variant'] ) ){
                $h1_var = ':' . travel_agency_pro_check_varient( $h1_font['font-family'], $h1_font['variant'] );    
            }else{
                $h1_var = '';
            }
        ...
        if ( 'off' !== $h2 && $ig_h2_font ) {
            if( ! empty( $h2_font['variant'] ) ){
                $h2_var = ':' . travel_agency_pro_check_varient( $h2_font['font-family'], $h2_font['variant'] );    
            }else{
                $h2_var = '';
            }
        ...
        if ( 'off' !== $h3 && $ig_h3_font ) {
            if( ! empty( $h3_font['variant'] ) ){
                $h3_var = ':' . travel_agency_pro_check_varient( $h3_font['font-family'], $h3_font['variant'] );    
            }else{
                $h3_var = '';
            }
        ...
        if ( 'off' !== $h4 && $ig_h4_font ) {
            if( ! empty( $h4_font['variant'] ) ){
                $h4_var = ':' . travel_agency_pro_check_varient( $h4_font['font-family'], $h4_font['variant'] );    
            }else{
                $h4_var = '';
            }
        ...
        if ( 'off' !== $h5 && $ig_h5_font ) {
            if( ! empty( $h5_font['variant'] ) ){
                $h5_var = ':' . travel_agency_pro_check_varient( $h5_font['font-family'], $h5_font['variant'] );    
            }else{
                $h5_var = '';
            }
        ...
        if ( 'off' !== $h6 && $ig_h6_font ) {
            if( ! empty( $h6_font['variant'] ) ){
                $h6_var = ':' . travel_agency_pro_check_varient( $h6_font['font-family'], $h6_font['variant'] );    
            }else{
                $h6_var = '';
            }
 ...
 * Get Google Fonts
*/
function travel_agency_pro_get_google_fonts(){
...
*/
function travel_agency_pro_get_websafe_font(){
...
*/
function travel_agency_pro_check_varient( $font_family = 'serif', $font_variants = 'regular', $body = false ){
    $variant = '';
    $var     = array();
    $google_fonts  = travel_agency_pro_get_google_fonts(); //Google Fonts
    $websafe_fonts = travel_agency_pro_get_websafe_font(); //Standard Web Safe Fonts
...
*/
function travel_agency_pro_get_css_variant( $font_variant ){
    $v_array = array(
		'100'       => array(
            'weight'    => '100',
            'style'     => 'normal'
...
*/
function travel_agency_pro_is_google_font( $font ){
    $return = false;
    $websafe_fonts = travel_agency_pro_get_websafe_font();
    if( $font ){
...
*/
function travel_agency_pro_get_fonts( $font_family, $font_variant ){
    
    $fonts = array();
    $websafe_fonts = travel_agency_pro_get_websafe_font(); //Web Safe Font
    
    if( $font_family ){
        if( travel_agency_pro_is_google_font( $font_family ) ){
            $fonts['font'] = esc_attr( $font_family ); //Google Font
            if( $font_variant ){
                $weight_style    = travel_agency_pro_get_css_variant( travel_agency_pro_check_varient( $font_family, $font_variant ) );
