...
function gmaps_results_initialize(){
    ...
    var myLatLng = {lat: parseFloat( tap_gdata.latitude ), lng: parseFloat( tap_gdata.longitude )};
    
    if( tap_gdata.scroll == '1' ){
        wheel = true;
        zoom  = false;
    }else{
        wheel = false;
    ...
    if( tap_gdata.ed_marker == '1' ){
    	// Place a marker in Atlanta
    	marker = new google.maps.Marker({
    		position : myLatLng,
    		map      : map,
            title    : tap_gdata.marker_title,
            content  : tap_gdata.marker_title
    	});
